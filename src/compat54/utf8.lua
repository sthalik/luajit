...
-- utf8 library.
--
-- Copyright (C) 2017-2020 Francois Perrad
--
...
local function argerror(caller, narg, extramsg, level)
    level = level or 1
    caller = getinfo and getinfo(level + 1, 'n').name or caller
...
local function utf8_len(s, i, j, lax)
  if type(s) ~= 'string' then
    typeerror('len', 1, s, 'string')
  end
  local len = #s
  i = i or 1
  if type(i) ~= 'number' then
    typeerror('len', 2, i, 'number')
  end
  local posi = posrelat(i, len)
  j = j or -1
  if type(j) ~= 'number' then
    typeerror('len', 3, j, 'number')
  end
  local posj = posrelat(j, len)
  if (1 > posi) or (posi > (len + 1)) then
    argerror('len', 2, "initial position out of bounds")
  end
  if posj > (len + 1) then
    argerror('len', 3, "final position out of bounds")
  end
...
local function utf8_codepoint(s, i, j, lax)
  if type(s) ~= 'string' then
    typeerror('codepoint', 1, s, 'string')
  end
  local len = #s
  i = i or 1
  if type(i) ~= 'number' then
    typeerror('codepoint', 2, i, 'number')
  end
  local posi = posrelat(i, len)
  j = j or posi
  if type(j) ~= 'number' then
    typeerror('codepoint', 3, j, 'number')
  end
  local pose = posrelat(j, len)
  if 1 > posi then
    argerror('codepoint', 2, "out of bounds")
  end
  if pose > len then
    argerror('codepoint', 3, "out of bounds")
  end
  local strict = not lax
  local t = {}
  while posi <= pose do
...
local function utf8_char(...)
  ...
  for i = 1, #arg do
    ...
    if type(v) ~= 'number' then
      typeerror('char', i, v, 'number')
    end
    if (v < 0) or (v > 0x7FFFFFFF) then
      argerror('char', i, "value out of range")
    end
    if v < 0x80 then
      t[#t+1] = v
    else
...
local function utf8_offset(s, n, i)
  ...
  if type(s) ~= 'string' then
    typeerror('offset', 1, s, 'string')
  end
  local len = #s
  if type(n) ~= 'number' then
    typeerror('offset', 2, n, 'number')
  end
  i = i or ((n >= 0) and 1 or (len + 1))
  if type(i) ~= 'number' then
    typeerror('offset', 3, i, 'number')
  end
  local posi = posrelat(i, len)
  if (1 > posi) or (posi > (len + 1)) then
    argerror('offset', 3, "position out of bounds")
  end
  if n == 0 then
...
local function utf8_codes(ss, lax)
  ...
  local function iter_codes(s, n)
    ...
    if type(s) ~= 'string' then
      typeerror('codes', 1, s, 'string')
    end
    local len = #s
    if type(n) ~= 'number' then
      typeerror('codes', 2, n, 'number')
    end
    if n < 0 then
      n = 0
    elseif n <= len then
