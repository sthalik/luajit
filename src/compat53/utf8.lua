...
local function utf8_len(s, i, j)
  ...
  if (1 > posi) or (posi > (len + 1)) then
    argerror('len', 2, "initial position out of string")
  end
  if posj > (len + 1) then
    argerror('len', 3, "final position out of string")
  end
  local n = 0
  while posi <= posj do
...
local function utf8_codepoint(s, i, j)
  ...
  if 1 > posi then
    argerror('codepoint', 2, "out of range")
  end
  if pose > len then
    argerror('codepoint', 3, "out of range")
  end
  local t = {}
  while posi <= pose do
    local res
...
local function utf8_offset(s, n, i)
  ...
  local posi = posrelat(i, len)
  if (1 > posi) or (posi > (len + 1)) then
    argerror('offset', 3, "position out of range")
  end
  if n == 0 then
